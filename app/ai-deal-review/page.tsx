...
const DEFAULT_FORM: Form = {
  prospectName: '', prospectType: 'Business Owner', assetType: 'LLC (S-Corp)',
  salePrice: '', costBasis: '', debtPayoff: '',
  txCosts: '', fedRate: '', stateRate: '',
  recapture: '', timeline: '', stage: 'Early Conversation',
};
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/deal-review', {
        ...
        body: JSON.stringify({ form, calc: { ...calc, salePrice: n(form.salePrice), costBasis: n(form.costBasis) } }),
      });
      if (!res.ok) throw new Error();
      const data = await res.json();
      setMemo(data.memo);
      logActivity(`Executive memo generated by Claude — ${form.prospectName}, ${form.assetType} (${fmt(n(form.salePrice))})`, 'var(--gold)');
    } catch {
      setMemo(buildMemo());
      logActivity(`Executive memo generated — ${form.prospectName}, ${form.assetType} (${fmt(n(form.salePrice))})`, 'var(--gold)');
      toast('Executive memo generated');
    } finally {
      setGenerating(false);
      setStep(3);
      incrementStat('memos');
  ...
  return (
    <div style={{ maxWidth: '1150px' }}>
          ...
          <div style={{ padding: '1.5rem', backgroundColor: 'var(--bg-primary)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {memo && (
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                <a href="/deal-calculator" style={{ textDecoration: 'none' }}><button className="btn-gold">Open Deal Calculator →</button></a>
                <a href="/powerpoint-builder" style={{ textDecoration: 'none' }}><button className="btn-secondary">Generate PowerPoint</button></a>
                <button className="btn-secondary" onClick={copyMemo}>Copy Memo</button>
      ...
      <div className="disclaimer" style={{ marginTop: '1.5rem' }}>
        ...
        All strategies and calculations must be reviewed by qualified professionals before any decisions are made.
      </div>
    </div>
  );
}
