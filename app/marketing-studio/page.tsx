...
const categoryToAudience: Record<string, string> = {
  'PE Partner': 'Private Equity Partner',
  'Investment Banker': 'Investment Banker',
  'Real Estate Broker': 'High-End Real Estate Broker',
  'CPA': 'CPA',
...
export default function MarketingStudioPage() {
  ...
  useEffect(() => {
    ...
    if (name || category) {
      ...
      setForm((f) => ({
        ...
        ...(mappedAudience ? { audience: mappedAudience } : {}),
      }));
    }
  }, []);
  ...
  const generate = async () => {
    ...
    try {
      ...
      setOutput(data);
      logActivity(`Outreach suite generated by Claude — ${form.audience}, ${form.goal} (${form.tone} tone)`, 'var(--purple)');
    } catch {
  ...
  return (
    <div style={{ maxWidth: '1200px' }}>
      <div style={{ marginBottom: '1.75rem' }}>
        <h1 className="section-title" style={{ fontSize: '1.375rem' }}>Referral Outreach Studio</h1>
        <p className="section-subtitle" style={{ maxWidth: '620px' }}>
          Generate educational materials, relationship outreach, and follow-up content for investment bankers, brokers, and CPAs — the 2,000 intermediaries in your referral network. Sophisticated tone, no mass-marketing feel. Designed to complement your HubSpot drip sequences with customized, deal-specific touchpoints.
        </p>
      </div>
